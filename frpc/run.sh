...
HEALTH_CHECK_URL=$(jq --raw-output '.health_check_url' $CONFIG_PATH)
...
ADMIN_ADDR=$(jq --raw-output '.admin_addr // empty' $CONFIG_PATH)
ADMIN_PORT=$(jq --raw-output '.admin_port // empty' $CONFIG_PATH)
